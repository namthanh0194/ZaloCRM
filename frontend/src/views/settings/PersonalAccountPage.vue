...
.page-title { font-size: 21px; font-weight: 700; color: var(--color-text, #141a24); margin: 0 0 3px; }
.page-desc { font-size: 13px; color: var(--color-text-muted, #6b7488); margin: 0; }

.card {
  background: var(--color-surface);
  border: 1px solid var(--color-border, #e7eaf0);
  border-radius: var(--radius-xl, 14px);
  box-shadow: var(--shadow-sm, 0 1px 3px rgba(20,26,36,.06));
  margin-bottom: 18px;
  overflow: hidden;
}
...
.card-head {
  padding: 14px 24px;
  border-bottom: 1px solid var(--color-border-strong, #eef1f6);
  display: flex; align-items: center; justify-content: space-between;
}
.card-head h3 {
  font-size: 13px; font-weight: 700; color: var(--color-text-muted, #6b7488);
...
.avatar-edit {
  ...
  background: var(--color-primary, #1786be); color: var(--color-on-primary);
  border: 2px solid #fff; cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  font-size: 14px; box-shadow: var(--shadow-sm, 0 1px 3px rgba(20,26,36,.06));
}
.avatar-edit:disabled { opacity: .6; cursor: default; }
.spin { display: inline-block; animation: sp 1s linear infinite; }
@keyframes sp { to { transform: rotate(360deg); } }
.hero-info { flex: 1; }
.hero-name { font-size: 20px; font-weight: 700; display: flex; align-items: center; gap: 10px; color: var(--color-text, #141a24); }
.hero-meta { font-size: 13px; color: var(--color-text-muted, #6b7488); margin-top: 6px; display: flex; gap: 16px; flex-wrap: wrap; }
.hero-meta b { color: var(--color-text-secondary, #475066); font-weight: 600; }
.avatar-actions { display: flex; gap: 8px; margin-top: 12px; }

/* Field rows */
.fields { padding-top: 8px; padding-bottom: 8px; }
.frow { display: flex; align-items: center; gap: 18px; padding: 13px 0; border-bottom: 1px solid var(--color-border-strong, #eef1f6); }
.frow:last-child { border-bottom: none; }
.flabel {
  width: 130px; flex-shrink: 0;
  font-size: 12px; font-weight: 600; color: var(--color-text-muted, #6b7488);
  text-transform: uppercase; letter-spacing: .04em;
}
.fval { font-size: 14px; color: var(--color-text, #141a24); font-weight: 500; flex: 1; }
.finput {
  flex: 1; max-width: 360px; padding: 9px 12px; font-size: 14px;
  border: 1px solid var(--color-border, #e7eaf0); border-radius: var(--radius-md, 8px);
  outline: none; font-family: inherit;
}
.finput:focus { border-color: var(--color-primary, #1786be); box-shadow: 0 0 0 3px rgba(23,134,190,.12); }
.locked {
  font-size: 11px; color: var(--color-text-disabled, #97a0b3);
  background: var(--color-surface-secondary, #f1f4f9); padding: 2px 8px; border-radius: var(--radius-pill, 999px);
}

/* Role chip */
.role-chip { font-size: 11.5px; font-weight: 700; padding: 3px 11px; border-radius: var(--radius-pill, 999px); }
.role-owner { background: #fdf4e3; color: #92610c; }
.role-admin { background: var(--color-primary-subtle, #e4f1f8); color: var(--color-primary-active, #0b5880); }
.role-member { background: #e8f6ed; color: #166534; }

/* Security */
.sec-row { display: flex; align-items: center; justify-content: space-between; }
.sec-lbl { font-size: 14px; font-weight: 600; color: var(--color-text, #141a24); }
.sec-sub { font-size: 12.5px; color: var(--color-text-muted, #6b7488); margin-top: 2px; }

/* Shortcuts */
.shortcuts { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.sc {
  display: flex; align-items: center; gap: 12px; padding: 14px 16px;
  border: 1px solid var(--color-border, #e7eaf0); border-radius: var(--radius-lg, 10px);
  cursor: pointer; background: var(--color-surface); text-decoration: none;
}
.sc:hover { border-color: var(--color-primary, #1786be); background: var(--color-primary-subtle, #f2f8fc); }
.sc-ic {
  width: 36px; height: 36px; border-radius: var(--radius-md, 8px); flex-shrink: 0;
  background: var(--color-primary-subtle, #e4f1f8); color: var(--color-primary-active, #0b5880);
  display: flex; align-items: center; justify-content: center; font-size: 18px;
}
.sc-txt { flex: 1; }
.sc-t { font-size: 13.5px; font-weight: 600; color: var(--color-text, #141a24); }
.sc-d { font-size: 12px; color: var(--color-text-muted, #6b7488); margin-top: 1px; }
.sc-arrow { color: var(--color-text-disabled, #97a0b3); font-size: 18px; }

/* Buttons */
.row-end { display: flex; justify-content: flex-end; gap: 10px; padding-top: 0; }
.btn {
  padding: 9px 16px; font-size: 13px; font-weight: 600;
  border-radius: var(--radius-md, 8px); border: 1px solid var(--color-border, #e7eaf0);
  background: var(--color-surface); color: var(--color-text, #141a24); cursor: pointer; font-family: inherit;
  display: inline-flex; align-items: center; gap: 7px;
}
.btn:hover:not(:disabled) { background: var(--color-surface-secondary, #f1f4f9); }
...
.btn-primary:hover:not(:disabled) { background: var(--color-primary-hover, #0f6fa0); }
.btn-danger { color: var(--color-danger, #f04438); border-color: #f5c4c0; }
.btn-danger:hover:not(:disabled) { background: var(--color-danger-subtle, #fdeceb); }
</style>
